...
void SendFrame::parsePaymentRequest(QString _request) {
    ...
    if(_request.startsWith("ccm://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "ccm:");
    }
    if(!_request.startsWith("ccm:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with ccm:"), QtCriticalMsg));
      return;
    }
  ...
  ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "ccmwL6nTg71T9qQbM3np3aPgimwD5GQCBEwca6YGVE5EDLTWmwxftvuR9nFiwDYTpJcwTq2DrekWKAKu1v8fd79b6xGwC3qqAS";
