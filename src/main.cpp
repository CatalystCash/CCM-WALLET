...
int main(int argc, char* argv[]) {
  ...
  QSettings ccmKey("HKEY_CLASSES_ROOT\\ccm", QSettings::NativeFormat);
  ccmKey.setValue(".", "ccm Wallet");
  ccmKey.setValue("URL Protocol", "");
  QSettings ccmOpenKey("HKEY_CLASSES_ROOT\\ccm\\shell\\open\\command", QSettings::NativeFormat);
  ccmOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
  ...
  //as root
  args << "-c" << "printf '[Desktop Entry]\\nName = ccm URL Handler\\nGenericName = ccm\\nComment = Handle URL Sheme ccm://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/ccm;\\nIcon = ccm-Wallet' | tee /usr/share/applications/ccm-handler.desktop";
